	 ...
	 let
	   ...
	   fun doit ss s =
	       if Substring.isEmpty s then ss
                 ...
                 let
                   val (sl,sr) = Substring.splitr (fn c => #"%"<>c) s
		 in
                   if Substring.isEmpty sl then sr::ss
		   else
                     let
                       val sl' =  Substring.trimr 1 sl
		     in
                       case Substring.first sr of
                           SOME #"1" => let val sr' = Substring.triml 1 sr
					in
                                          doit (Substring.full src::sr'::ss) sl'
					end
			 | SOME #"2" => let val sr' = Substring.triml 1 sr
					in
                                          doit (Substring.full dst::sr'::ss) sl'
					end
